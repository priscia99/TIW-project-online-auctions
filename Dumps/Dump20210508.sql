...
INSERT INTO `auction` VALUES (1,3,1,10,99.99,'2022-04-22 10:00:17','2021-05-07 21:32:14',1),(2,4,1,10,199.99,'2022-10-22 10:00:00','2021-05-07 21:33:15',1);
...
--
-- Temporary view structure for view `close_auctions`
--

DROP TABLE IF EXISTS `close_auctions`;
/*!50001 DROP VIEW IF EXISTS `close_auctions`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `close_auctions` AS SELECT 
 1 AS `id_auction`,
 1 AS `id_item`,
 1 AS `id_seller`,
 1 AS `minimum_rise`,
...
INSERT INTO `item` VALUES (1,'Oscilloscopio','Alto, basso, alto, auto, amp. verticale','unknown'),(2,'oscilloscopio','Alto, basso, alto, auto, amp. verticale','unknown'),(3,'oscilloscopio','Alto, basso, alto, auto, amp. verticale','unknown'),(4,'Multimetro','a doppia rampa flash','unknown');
...
INSERT INTO `user` VALUES (1,'pizza',_binary '3323469a03f23afab163e37863c39565931da4b1e6fee504c861e93fc0670cd6','pizza','pizza','pizza','pizza','pizza');
...
/*!50013 DEFINER=`workbench`@`%` SQL SECURITY DEFINER */
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`workbench`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `auctions_open_details` AS select `ai`.`id_auction` AS `id_auction`,`ai`.`id_item` AS `id_item`,`ai`.`id_seller` AS `id_seller`,`ai`.`minimum_rise` AS `minimum_rise`,`ai`.`starting_price` AS `starting_price`,`ai`.`end` AS `end`,`ai`.`creation` AS `creation`,`ai`.`open` AS `open`,`ai`.`name` AS `name`,`ai`.`description` AS `description`,`ai`.`image_filename` AS `image_filename`,`b`.`id_bid` AS `id_bid`,`b`.`id_bidder` AS `id_bidder`,`b`.`price` AS `price`,`b`.`bid_time` AS `bid_time` from (`auction_item` `ai` left join `bid` `b` on((`ai`.`id_auction` = `b`.`id_auction`))) where (`ai`.`open` = true) group by `ai`.`id_auction` order by `b`.`bid_time` desc */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `close_auctions`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`workbench`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `close_auctions` AS select `ai`.`id_auction` AS `id_auction`,`ai`.`id_item` AS `id_item`,`ai`.`id_seller` AS `id_seller`,`ai`.`minimum_rise` AS `minimum_rise`,`ai`.`starting_price` AS `starting_price`,`ai`.`end` AS `end`,`ai`.`creation` AS `creation`,`ai`.`open` AS `open`,`ai`.`name` AS `name`,`ai`.`description` AS `description`,`ai`.`image_filename` AS `image_filename`,`mb`.`id_bid` AS `id_max_bid`,`mb`.`id_bidder` AS `id_max_bidder`,`mb`.`price` AS `max_price`,`mb`.`bid_time` AS `max_bid_time` from (`auction_item` `ai` left join `max_bids` `mb` on((`mb`.`id_auction` = `ai`.`id_auction`))) where (`ai`.`open` = false) order by `ai`.`end` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`workbench`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `open_auctions` AS select `ai`.`id_auction` AS `id_auction`,`ai`.`id_item` AS `id_item`,`ai`.`id_seller` AS `id_seller`,`ai`.`minimum_rise` AS `minimum_rise`,`ai`.`starting_price` AS `starting_price`,`ai`.`end` AS `end`,`ai`.`creation` AS `creation`,`ai`.`open` AS `open`,`ai`.`name` AS `name`,`ai`.`description` AS `description`,`ai`.`image_filename` AS `image_filename`,`mb`.`id_bid` AS `id_max_bid`,`mb`.`id_bidder` AS `id_max_bidder`,`mb`.`price` AS `max_price`,`mb`.`bid_time` AS `max_bid_time` from (`auction_item` `ai` left join `max_bids` `mb` on((`mb`.`id_auction` = `ai`.`id_auction`))) where (`ai`.`open` = true) order by `ai`.`end` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2021-05-08 12:21:23
